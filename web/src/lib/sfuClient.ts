...
export async function sfuHandshake(roomId: string)
{
    ...
    {
        socket.emit("sfu:produce", { roomId, transportId: sendTransport.id, kind, rtpParameters, appData }, (res: any) =>
        {
...
): Promise<mediasoupClient.types.Consumer>
{
    ...
    }>("sfu:consume", { roomId, transportId: recvTransport.id, producerId, rtpCapabilities: device.rtpCapabilities });
...
export function onNewproducer(
    handler: (data: { producerId: string, kind: "audio" | "video" }) => void
)
{
...
export function onConsumerClosed(
    handler: (data: { consumerId: string }) => void
)
{
